...
select 
ROUND((select COUNT(*) from CTE where rnk = 1 and order_date = customer_pref_delivery_date)*100/
(select count(distinct customer_id) from CTE),2) as immediate_percentage